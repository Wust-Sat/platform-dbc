...
from platform_dbc.modules import (MODULES, MessageType, Module,
                                  get_active_modules)
...
def encode_heartbeat(
    ...
    try:
        data = db.encode_message(
            f"{module_name}_Heartbeat", {"unix_timestamp": timestamp}
        )
...
def decode_message(
    db: cantools.database.Database, message_name: str, data: bytes
) -> Optional[dict]:
    """
